---
title: "Modul 05 - Introduction to Descriptive Analytics"
author: "Roni Yunis"
date: "3/20/2021"
output: pdf_document
---
...
```{r}
# nilai minimum dari Quantity
min(asuransi$age)

# nilai maximum dari UnitPrice
max(asuransi$age)
```
...
Kalau ada dalam data kita data *missing value (NA)*, maka fungsi `mean(retail$UnitPrice, na.rm = TRUE)` bisa kita gunakan, artinya data NA itu tidak termasuk dalam rata-rata yang kita cari.
...
Untuk melihat kategori dari variabel yang ada bisa mengguakan fungsi `table()`. Misalnya kita ingin melihat kategori yang ada pada variabel *sex*, maka fungsi `table()` bisa dituliskan seperti ini.
